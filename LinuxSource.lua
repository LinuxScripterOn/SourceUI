...
function Linux.Create(config)
    ...
    function LinuxLib.Tab(config)
        ...
        function TabElements.Dropdown(config)
                ...
                if IsOpen then
                    for _, opt in pairs(Options) do
                        local OptBtn = Linux.Instance("TextButton", {
                            ...
                            Font = Enum.Font.Code,
                            Text = tostring(opt),
                            TextColor3 = opt == SelectedValue and Color3.fromRGB(220, 220, 220) or Color3.fromRGB(130, 130, 130),
                            TextSize = 14,
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            ...
                            for _, btn in pairs(DropFrame:GetChildren()) do
                                if btn:IsA("TextButton") then
                                    btn.TextColor3 = btn.Text == tostring(opt) and Color3.fromRGB(220, 220, 220) or Color3.fromRGB(130, 130, 130)
                                end
                            end
            ...
            local function SetValue(value)
                if table.find(Options, value) then
                    ...
                    for _, btn in pairs(DropFrame:GetChildren()) do
                        if btn:IsA("TextButton") then
                            btn.TextColor3 = btn.Text == tostring(value) and Color3.fromRGB(220, 220, 220) or Color3.fromRGB(130, 130, 130)
                        end
                    end
                    Linux:SaveConfig("Dropdown_" .. config.Name, SelectedValue, shouldSave, configFolder)
        ...
        function TabElements.Slider(config)
            local container = config.Container == 2 and Container1 or Container2
            local elementOrder = config.Container == 2 and elementOrder2 or elementOrder1
            if config.Container == 2 then
                elementOrder2 = elementOrder2 + 1
            else
